...
use crate::utils::bench::{calculate_std_dev, time_seed, shuffle_with_rng, SeededRng};
...
pub fn run_all_algorithms_randomized(
    ...
    for task in &tasks {
        if seen_closures[task.closure_idx] { continue; }
    ...
    GroupedResults { results: grouped, raw_data }
}
...
pub fn export_csv(path: &str, data: &[RawTimingData]) -> std::io::Result<()> {
    ...
    writeln!(file, "algorithm,variant,compiler,size,iteration,time_ns,result")?;
