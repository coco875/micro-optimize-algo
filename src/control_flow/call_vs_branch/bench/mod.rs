...
fn generate_test_data(size: usize) -> Vec<u32> {
    ...
    for _ in 0..size {
        ...
        data.push((seed >> 32) as u32 % 512); // Values 0-511 for good distribution
    }
    data
}
...
fn calculate_std_dev(times: &[Duration], mean: Duration) -> Duration {
    ...
    let variance: f64 = times.iter()
        ...
        .sum::<f64>() / (times.len() - 1) as f64;  // Sample variance (n-1)
...
fn benchmark_function(func: TestFn, data: &[u32], iterations: usize) -> (Duration, Duration, Duration, Duration, u32) {
    // Warmup
