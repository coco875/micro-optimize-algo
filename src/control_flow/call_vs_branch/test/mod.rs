...
pub fn verify_all() -> Result<(), String> {
    let test_values: Vec<u32> = vec![
        0, 1, 2, 5, 10, 50, 100, 255, 500, 1000, 10000
    ];
...
mod tests {
    ...
    fn test_expected_computation() {
        ...
        for variant in &variants {
            assert_eq!((variant.func)(5), 400, "{}: process(5) should be 400", variant.name);
        }
    }
    ...
    fn test_zero() {
        ...
        let variants = get_variants();
        for variant in &variants {
            assert_eq!((variant.func)(0), 100, "{}: process(0) should be 100", variant.name);
        }
    }
}
