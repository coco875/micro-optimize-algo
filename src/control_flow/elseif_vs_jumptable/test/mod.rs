...
mod tests {
    ...
    fn test_all_opcodes() {
        ...
        for variant in &variants {
            ...
            assert_eq!((variant.func)(8, value), 0, "{}: invalid op", variant.name);
        }
    }
    ...
    fn test_edge_values() {
        ...
        for variant in &variants {
            // Test with 0
            assert_eq!((variant.func)(0, 0), 0, "{}: 0 * anything = 0", variant.name);
            
            // Test with 1
            assert_eq!((variant.func)(0, 1), 1, "{}: identity of 1", variant.name);
            
            // Test with large value (avoiding overflow)
            assert_eq!((variant.func)(1, 1000000), 2000000, "{}: large value", variant.name);
        }
    }
}
